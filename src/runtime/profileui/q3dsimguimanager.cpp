...
void Q3DSImguiManager::initialize(Qt3DCore::QEntity *rootEntity)
{
...
// Q3DSImguiManager references) but the Q3DSImguiManager instance will be reused later
// on. Must be followed by a call to initialize().
void Q3DSImguiManager::releaseResources()
{
...
void Q3DSImguiManager::resizePool(CmdListEntry *e, int newSize)
{
...
void Q3DSImguiManager::updateGeometry(CmdListEntry *e, int idx, uint elemCount, int vertexCount,
...
void Q3DSImguiManager::update3D()
{
...
Qt3DRender::QMaterial *Q3DSImguiManager::buildMaterial(Qt3DRender::QScissorTest **scissor)
{
...
class Q3DSImguiInputEventFilter : public QObject
{
public:
    Q3DSImguiInputEventFilter()
    {
...
bool Q3DSImguiInputEventFilter::eventFilter(QObject *, QEvent *event)
{
    if (!enabled)
        return false;
...
Q3DSImguiManager::~Q3DSImguiManager()
{
    delete m_inputEventFilter;
}

void Q3DSImguiManager::setInputEventSource(QObject *src)
{
    ...
    if (!m_inputEventFilter)
        m_inputEventFilter = new Q3DSImguiInputEventFilter;

    m_inputEventSource->installEventFilter(m_inputEventFilter);
}

void Q3DSImguiManager::updateInput()
{
    if (!m_inputEventFilter)
        return;
...
void Q3DSImguiManager::setEnabled(bool enabled)
{
