...
{
    return getValue<float>(d_ptr->m_values, property, 0.0);
}
...
{
    return getMinimum<float>(d_ptr->m_values, property, 0.0);
}
...
{
    return getMaximum<float>(d_ptr->m_values, property, 0.0);
}
...
{
    const QtFloatPropertyManagerPrivate::PropertyValueMap::const_iterator it = d_ptr->m_values.constFind(property);
    if (it == d_ptr->m_values.constEnd())
        return QString();
    return QString::number(it.value().val, 'f', it.value().decimals);
}
...
{
    void (QtFloatPropertyManagerPrivate::*setSubPropertyValue)(QtProperty *, float) = 0;
...
{
    const QtFloatPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
...
{
    void (QtFloatPropertyManagerPrivate::*setSubPropertyRange)(QtProperty *, float, float, float) = 0;
...
void QtVector3DPropertyManager::initializeProperty(QtProperty *property)
{
    ...
    d_ptr->m_doublePropertyManager->setRange(xProp, -DBL_MAX, DBL_MAX);
    d_ptr->m_propertyToX[property] = xProp;
    d_ptr->m_xToProperty[xProp] = property;
    property->addSubProperty(xProp);
    ...
    d_ptr->m_doublePropertyManager->setRange(yProp, -DBL_MAX, DBL_MAX);
    d_ptr->m_propertyToY[property] = yProp;
    d_ptr->m_yToProperty[yProp] = property;
    property->addSubProperty(yProp);
    ...
    d_ptr->m_doublePropertyManager->setRange(zProp, -DBL_MAX, DBL_MAX);
